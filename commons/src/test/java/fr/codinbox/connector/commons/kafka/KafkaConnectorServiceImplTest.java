...
class KafkaConnectorServiceImplTest {
    ...
    @BeforeEach
    void setUp() {
        service = new KafkaConnectorServiceImpl(Logger.getLogger("test"));
    }
    ...
    void init_missingConfig_exitOnFailure_throws() {
        env.set("CONNECTOR_KAFKA_BAD_EXIT_ON_FAILURE", "true");
        env.set("CONNECTOR_KAFKA_BAD_OTHER", "value");
    ...
    void init_missingConfig_noExitOnFailure_skips() throws ConnectionInitException {
        env.set("CONNECTOR_KAFKA_BAD_EXIT_ON_FAILURE", "false");
        env.set("CONNECTOR_KAFKA_BAD_OTHER", "value");
